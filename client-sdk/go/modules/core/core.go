...
type V1 interface {
	...
	EstimateGasForCaller(ctx context.Context, round uint64, caller CallerAddress, tx *types.Transaction) (uint64, error)
...
// Implements V1.
func (a *v1) EstimateGasForCaller(ctx context.Context, round uint64, caller CallerAddress, tx *types.Transaction) (uint64, error) {
	var gas uint64
